...
function CanvasEditor({ onReturn }: { onReturn: () => void }) {
  ...
  const [validationState, setValidationState] = useState<{ status: 'idle' | 'error' | 'success', message: string }>({ status: 'idle', message: '' });

  // History state for undo/redo
  const [history, setHistory] = useState<{ nodes: Node[], edges: Edge[] }[]>([{ nodes: initialNodes, edges: [] }]);
  const [historyIndex, setHistoryIndex] = useState(0);

  // Take snapshot of state for undo/redo
  ...
  const validateAndSave = () => {
    ...
    const invalidStepNodes = nodes.filter(n => n.type === 'step' && !n.data.tool);
    if (invalidStepNodes.length > 0) {
      setValidationState({ status: 'error', message: `Node "${invalidStepNodes[0].data.label}" is missing a required Tool Name.` });
      setSelectedNode(invalidStepNodes[0]);
      return;
    }

    // 2. Check for orphaned nodes
    // An orphaned node is one that is not connected to any edge, unless it's the only node
    if (nodes.length > 1) {
      ...
      if (orphanedNodes.length > 0) {
        setValidationState({ status: 'error', message: `Orphaned node detected: "${orphanedNodes[0].data.label}". Connect or remove it.` });
    ...
    if (hasCycle) {
      setValidationState({ status: 'error', message: 'Infinite loop (cycle) detected in the graph.' });
      return;
    }
  ...
  return (
    <div className="flex h-full w-full bg-zinc-50 relative overflow-hidden">
      ...
      <div className="flex flex-col flex-1 h-full relative" ref={reactFlowWrapper}>
        {/* Toolbar */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-4 bg-white px-4 py-2 rounded-full border shadow-sm">
          <div className="flex items-center gap-2 border-r pr-4">
            <button onClick={onReturn} className="text-sm font-medium text-zinc-600 hover:text-zinc-900 transition-colors">
              Exit Canvas
            </button>
          </div>
          
          <div className="flex items-center gap-2 border-r pr-4 min-w-[120px]">
